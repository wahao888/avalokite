...
// 第 2 號翡翠莊園綠標：NT$2,000（本站最貴的一支，拿來驗多包數的乘法）
const PRICEY = "esmeralda-green-label";
...
describe("豆單與紙本一致（2026 九月豆單）", () => {
  ...
  const SHEET: [number, string, number][] = [
    [1, "極品夏威夷可娜 Extra Fancy 等級", 2000],
    [2, "空運巴拿馬藝伎 翡翠莊園綠標", 2000],
    [3, "空運牙買加藍山 No.1 克里斯戴爾莊園", 1700],
    [4, "巴拿馬藝伎 —「90+」Ninety Plus 雙重厭氧", 1200],
    [5, "衣索比亞藝伎 凱蕾莊園", 700],
    [6, "巴拿馬「90+」經典藝伎", 1200],
    [7, "最佳巴拿馬藝伎 阿爾鐵里莊園", 1600],
    [8, "瓜地馬拉小藍莓莊園藝伎", 1200],
    [9, "衣索比亞耶加雪菲 G1・Aricha Adorsi 處理廠", 400],
    [10, "衣索比亞 Lucy 藝伎之母", 600],
    [11, "肯亞 多門 AA TOP", 400],
    [12, "蘇門達臘林東藍眼曼特林 G1 TP", 400],
    [13, "哥倫比亞卡斯提優・天堂水果", 600],
    [14, "花神來了 — 瓜地馬拉", 400],
    [15, "秘魯 SHB・北方之花", 400],
  ];
  ...
  it.each(SHEET)("第 %i 號 %s：售價與紙本相符", (no, name, price) => {
...
describe("購物車結算", () => {
  ...
  it("線上不做任何折扣：小計就是各行相加", () => {
    // 客戶 2026-09-09 指示網路上不要特價。紙本豆單的三包整組價不套用在線上，
    // 所以買幾包都是原價乘法——這條在守「有人日後把折扣加回來卻沒告知」。
    const t = priceCart([line(PRICEY, 3), line(PLAIN, 2)], "transfer");
    expect(t.lines[0]!.amount).toBe(getBean(PRICEY)!.price * 3);
    expect(t.lines[1]!.amount).toBe(400 * 2);
    expect(t.subtotal).toBe(t.lines.reduce((a, l) => a + l.amount, 0));
  });
...
describe("運費與付款（2026-09-02 客戶確認）", () => {
  it("運費 100、滿 2000 免運、貨到付款免手續費", () => {
    expect(SHIPPING_FEE).toBe(100);
    expect(FREE_SHIPPING_OVER).toBe(2000);
    // 兩種付款方式的總額必須一致——貨到付款不加收
    const a = priceCart([line(PLAIN, 1)], "cod");
    const b = priceCart([line(PLAIN, 1)], "transfer");
    expect(a.total).toBe(b.total);
    expect(a.total).toBe(400 + 100);
  });

  it("Totals 不再有折扣相關欄位（線上不做特價）", () => {
    const t = priceCart([line(PLAIN, 1)], "cod");
    for (const k of ["codFee", "bundles", "discount", "listTotal"]) {
      expect(k in t, `Totals 不該有 ${k}`).toBe(false);
    }
  });
});
