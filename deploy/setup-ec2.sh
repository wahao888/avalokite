...
echo "   3. snap install --classic certbot"
echo "      certbot --nginx --expand \$(sed 's/^/-d /;/^#/d;/^\$/d' $APP_DIR/app/deploy/domains.txt | tr '\\n' ' ')"
echo "   4. GoDaddy DNS 加 A 記錄 '*' → 本機 IP（客戶站子網域用，見 DEPLOY.md 3.5）"
